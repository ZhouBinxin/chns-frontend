<template>
  <div class="selectType">
    ...
    <div class="healthSelect">
      ...
      <div class='selectContent'>
        <el-checkbox-group v-model="checkboxGroup1" size="medium">
          <el-checkbox-button @change="showHealthDetail(index)" v-for="(item, index) in healthTypes" :label="item" :key="item">{{ item }}
          </el-checkbox-button>
        </el-checkbox-group>
      </div>
      <div>
        <el-row class="detail-select-box">
          <el-col :span="8" v-if="detailItem.ifShow" v-for="(detailItem, detailIndex) in healthDetailInfo" :key="detailIndex">
            <span class="healthTitle">{{ detailItem.title }}</span>
            <el-select v-if="detailIndex !== 4" v-model="detailItem.curr" clearable multiple placeholder="请选择"
              @change="healthChange(detailItem, detailIndex)">
              <el-option v-for="item in detailItem.detailLists" :key="item" :label="item" :value="item">
              </el-option>
            </el-select>
            <el-cascader v-if="detailIndex === 4" v-model="detailItem.curr" @change="healthChange(detailItem, detailIndex)" placeholder="请选择"
              :options="detailItem.detailLists" :props="{ expandTrigger: 'hover' }"></el-cascader>
          </el-col>
        </el-row>
      </div>
    </div>
    ...
    <div class="propertySelect">
      ...
      <div class='selectContent'>
        <el-checkbox-group v-model="checkboxGroup1" size="medium">
          <el-checkbox-button @change="showPropertyDetail(index)" v-for="(item, index) in propertyTypes" :label="item" :key="item">{{ item }}
          </el-checkbox-button>
        </el-checkbox-group>
      </div>
      <div>
        <el-row class="detail-select-box">
          <el-col :span="24" v-if="detailItem.ifShow" v-for="(detailItem, detailIndex) in propertyDetailInfo" :key="detailIndex">
            <span class="healthTitle">{{ detailItem.title }}</span>
            <el-select v-if="selectItem.itemName !== '饮用水' && selectItem.itemName !== '过滤水'" multiple v-model="selectItem.curr" clearable
              placeholder="请选择" v-for="(selectItem, selectIndex) in detailItem.itemLists" :key="selectIndex"
              @change="propertyChange(detailItem, selectItem, selectIndex)">
              <el-option v-for="item in selectItem.detailLists" :key="item" :label="item" :value="item">
              </el-option>
            </el-select>
            <el-select v-if="selectItem.itemName === '饮用水' || selectItem.itemName === '过滤水'" v-model="selectItem.curr" clearable placeholder="请选择"
...
export default {
  ...
  methods: {
    ...
    showHealthDetail(index) {
      ...
      if (
        temp.title in this.healthFilterInfoObj &&
        !this.healthDetailInfo[index].ifShow
      ) {
    ...
    showPropertyDetail(index) {
      ...
      // 当该项数据在用户数据中，但用户未选择时，需要进行删除
      if (
        temp.title in this.propertyFilterInfoObj &&
        !this.propertyDetailInfo[index].ifShow
      ) {
